...
public class BMICheck {
	...
	private void load() {
		...
		try {
			...
			/*
			 * Set<Integer> set = map.keySet();
			 * System.out.println("번호\t키\t몸무게\t결과\t판정"); for (Integer number :
			 * set) { double length = map.get(number).getLength(); double weight
			 * = map.get(number).getWeight(); double result =
			 * map.get(number).getResult(); String you =
			 * map.get(number).getYou();
			 * 
			 * System.out.printf("%s\t%.2f\t%.2f\t%.2f\t%s\n", number, length,
			 * weight, result, you); }
			 */
